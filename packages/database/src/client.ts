...
export async function initDatabaseSchema(): Promise<void> {
  ...
  try {
    await db.$executeRawUnsafe(`
      ...
      DO $$ BEGIN
        CREATE TYPE "AccountStatus" AS ENUM ('ACTIVE', 'DISCONNECTED', 'REVOKED', 'REFRESH_FAILED');
      EXCEPTION
        WHEN duplicate_object THEN null;
      END $$;

      DO $$ BEGIN
        CREATE TYPE "MeetingStatus" AS ENUM ('CREATED', 'SCHEDULED', 'STARTING', 'AUTHENTICATING', 'SDK_INITIALIZING', 'JOINING', 'WAITING_ROOM', 'CONNECTED', 'RECONNECTING', 'STOPPING', 'COMPLETED', 'FAILED', 'CANCELLED');
      EXCEPTION
        WHEN duplicate_object THEN null;
      END $$;

      DO $$ BEGIN
        CREATE TYPE "SessionStatus" AS ENUM ('CREATED', 'STARTING', 'CONNECTED', 'RECONNECTING', 'ENDING', 'COMPLETED', 'FAILED');
      EXCEPTION
        WHEN duplicate_object THEN null;
      END $$;

      DO $$ BEGIN
        CREATE TYPE "AuthType" AS ENUM ('ZAK', 'OBF');
      EXCEPTION
        WHEN duplicate_object THEN null;
      END $$;
    ...
    console.error('initDatabaseSchema error (proceeding):', err);
  }
}
