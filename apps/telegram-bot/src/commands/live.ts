...
export async function liveCommand(ctx: BotContext): Promise<void> {
  ...
  // If ADMIN_API_KEY is configured, the Live Screen's control/status/screenshot
  // endpoints require it — append it here so the button the user taps actually
  // works instead of silently failing with 403s.
  const adminKey = process.env['ADMIN_API_KEY']?.trim();
  const linkUrl = adminKey ? `${liveMonitorUrl}?key=${encodeURIComponent(adminKey)}` : liveMonitorUrl;
